...
<a href={resolve('/distros/[slug]', { slug: distro.slug })} class="block h-full">
	<Card class="h-full transition-shadow hover:shadow-md">
		...
		<CardContent class="mt-auto flex flex-wrap gap-2">
			<Badge variant="secondary">{formatEnumLabel(distro.releaseModel)}</Badge>
			<Badge variant="outline">{formatEnumLabel(distro.defaultDesktopEnvironment)}</Badge>
