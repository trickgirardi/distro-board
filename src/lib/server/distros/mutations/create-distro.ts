...
import { distros, type NewDistro } from '../../db/schema';
...
export async function createDistro(db: ReturnType<typeof createDb>, input: DistroInput) {
	const [existing] = await db
		.select({ id: distros.id })
		.from(distros)
		.where(eq(distros.slug, input.slug))
		.limit(1);
	...
	const [created] = await db
		.insert(distros)
		.values(distroData as Omit<NewDistro, 'id' | 'createdAt' | 'updatedAt'>)
