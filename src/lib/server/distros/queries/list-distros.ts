import { and, arrayContains, asc, eq, ilike, inArray, or, type SQL } from 'drizzle-orm';
...
export function parseListDistrosFilters(
	...
	if (isValidEnumValue(desktopEnvironmentEnum.enumValues, query.desktopEnvironment)) {
...
export async function listDistros(
	...
	if (filters.search) {
		...
		conditions.push(or(ilike(distros.name, term), ilike(distros.shortDescription, term))!);
	}
	...
	if (filters.desktopEnvironment) {
		conditions.push(
			or(
				...
				arrayContains(distros.supportedDesktopEnvironments, [filters.desktopEnvironment])
			)!
		);
	}

	if (filters.packageManager) {
		conditions.push(arrayContains(distros.packageManagers, [filters.packageManager]));
	}
		...
		conditions.push(
			inArray(
				...
				db
					...
					.where(eq(distroTargetAudiences.targetAudience, filters.targetAudience))
			)
		);
	}
